...
<form action="{{ route('invoice.update', $invoice_data->invoiceID ) }}" method="POST">
  @csrf
  <div class="mb-3 mt-3">
    <label for="invoiceId" class="form-label">Invoice ID:</label>
    <strong>{{ $invoice_data->invoiceID }}</strong>
  </div>
  <div class="mb-3">
    <label for="subject" class="form-label">Subject:</label>
    <input type="text" class="form-control" id="subject" placeholder="Enter Subject" name="subject" value="{{ old('subject', $invoice_data->subject) }}">
  ...
  <div class="mb-3">
    ...
    <select class="form-control" name="fromId" id="fromId">
      ...
      @foreach ($company_datas as $key=>$item)
          <option value="{{ $key }}" {{ old('fromId', $invoice_data->fromId) == $key ? 'selected' : '' }}>{{ $item }}</option>
      @endforeach
    </select>
  ...
  <div class="mb-3">
    ...
    <select class="form-control" name="forId" id="forId">
      ...
      @foreach ($client_datas as $key=>$item)
          <option value="{{ $key }}" {{ old('forId', $invoice_data->forId) == $key ? 'selected' : '' }}>{{ $item }}</option>
      @endforeach
    </select>
  ...
  <div class="mb-3">
    ...
    <input type="date" class="form-control" id="dueDate" placeholder="Enter Due Date" name="dueDate" value="{{ old('dueDate',$invoice_data->dueDate) }}">
